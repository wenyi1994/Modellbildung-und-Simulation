...
while t4(t_count)<ts
    ...
    k1h(:,t_count)=h_opt.*(SystMatr*x_RuKu_h(:,t_count));
    k2h(:,t_count)=h_opt.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k1h(:,t_count)));
    k3h(:,t_count)=h_opt.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k2h(:,t_count)));
    k4h(:,t_count)=h_opt.*(SystMatr*(x_RuKu_h(:,t_count)+k3h(:,t_count)));
    ...
    if delta > F_opt
        h_opt = ((2^p(2)-1)/2^p(2)*F_opt/delta)^(1/p(2))*h_opt;
        t_temp = t4(t_count);
        t_count = 1;
