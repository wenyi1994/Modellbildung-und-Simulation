...
%       - Glabale Diskratisierungsfehler nach max. Anzahl der Schritte mit
%         optimierten 'h' und vorgegebener Fehler
...
ts=5;                               % s             %% Laufzeit
F_opt=0.001;                                        %% vorgegebener Fehler
...
while t2(t_count) < ts
    ...
    x_Eu_expl_h(:,t_count+1)=x_Eu_expl_h(:,t_count)+h_opt_EE.*(SystMatr*x_Eu_expl_h(:,t_count));
    delta=abs(x_Eu_expl_h(1,t_count+1)-pi/6*cos(sqrt(g/l)*(t2(t_count)+h_opt_EE)));
    if delta > F_opt
        h_opt_EE = ((2^p(1)-1)/2^p(1)*F_opt/delta)^(1/p(1))*h_opt_EE;
        t_temp = t2(t_count);
        t_count = 1;
        while t2(t_count) < t_temp
            x_Eu_expl_h(:,t_count+1)=x_Eu_expl_h(:,t_count)+h_opt_EE.*(SystMatr*x_Eu_expl_h(:,t_count));
            t2(t_count+1)=t2(t_count)+h_opt_EE;
            t_count=t_count+1;
        end   
    else
        t2(t_count+1)=t2(t_count)+h_opt_EE;
...
AnzahlSchritte2=t_count;

h_opt_EI = h;
t_count = 1;
while t3(t_count)<ts
    %% Euler-Imp mit optimierten h
    x_Eu_impl_h(:,t_count+1) =(eye(2)-h_opt_EI.*SystMatr)\x_Eu_impl_h(:,t_count);
    delta=abs(x_Eu_impl_h(1,t_count+1)-pi/6*cos(sqrt(g/l)*(t3(t_count)+h_opt_EI)));
    if delta > F_opt
        h_opt_EI = ((2^p(1)-1)/2^p(1)*F_opt/delta)^(1/p(1))*h_opt_EI;
        t_temp = t3(t_count);
        t_count = 1;
        while t3(t_count) < t_temp
            x_Eu_impl_h(:,t_count+1) =(eye(2)-h_opt_EI.*SystMatr)\x_Eu_impl_h(:,t_count);
            t3(t_count+1)=t3(t_count)+h_opt_EI;
            t_count=t_count+1;
        end
    else
        t3(t_count+1)=t3(t_count)+h_opt_EI;
...
AnzahlSchritte3=t_count;

h_opt_RK = h;
t_count = 1;
while t4(t_count)<ts
    %% Runge-Kutta mit optimierten h
    k1h(:,t_count)=h_opt_RK.*(SystMatr*x_RuKu_h(:,t_count));
    k2h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k1h(:,t_count)));
    k3h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k2h(:,t_count)));
    k4h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+k3h(:,t_count)));
    x_RuKu_h(:,t_count+1) = x_RuKu_h(:,t_count)+1/6.*(k1h(:,t_count)+2*k2h(:,t_count)+2*k3h(:,t_count)+k4h(:,t_count));
    delta=abs(x_RuKu_h(1,t_count+1)-pi/6*cos(sqrt(g/l)*(t4(t_count)+h_opt_RK)));
    if delta > F_opt
        h_opt_RK = ((2^p(2)-1)/2^p(2)*F_opt/delta)^(1/p(2))*h_opt_RK;
        t_temp = t4(t_count);
        t_count = 1;
        while t4(t_count) < t_temp
            k1h(:,t_count)=h_opt_RK.*(SystMatr*x_RuKu_h(:,t_count));
            k2h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k1h(:,t_count)));
            k3h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+0.5.*k2h(:,t_count)));
            k4h(:,t_count)=h_opt_RK.*(SystMatr*(x_RuKu_h(:,t_count)+k3h(:,t_count)));
            x_RuKu_h(:,t_count+1) = x_RuKu_h(:,t_count)+1/6.*(k1h(:,t_count)+2*k2h(:,t_count)+2*k3h(:,t_count)+k4h(:,t_count));
            t4(t_count+1)=t4(t_count)+h_opt_RK;
            t_count=t_count+1;
        end
    else
        t4(t_count+1)=t4(t_count)+h_opt_RK;
...
AnzahlSchritte4=t_count;
disp('Berechnungszeit: Numerische Verfahren mit optimierten ''h''');
disp(['                                    ',num2str(toc),' seconds']);
disp('----------------------------------------------------------------------');

%% Fehler mit optimierten h
x_exakt2 = [pi/6*cos(sqrt(g/l)*t2);-pi/6*sqrt(g/l)*sin(sqrt(g/l)*t2)];
x_exakt3 = [pi/6*cos(sqrt(g/l)*t3);-pi/6*sqrt(g/l)*sin(sqrt(g/l)*t3)];
x_exakt4 = [pi/6*cos(sqrt(g/l)*t4);-pi/6*sqrt(g/l)*sin(sqrt(g/l)*t4)];

temp_Eu_ex = x_Eu_expl_h(1,:)-x_exakt2(1,:);
temp_Eu_im = x_Eu_impl_h(1,:)-x_exakt3(1,:);
temp_RuKu = x_RuKu_h(1,:)-x_exakt4(1,:);
    
temp_1 = max(temp_Eu_ex);
temp_2 = max(abs(temp_Eu_ex));
e_Eu_expl_h = [temp_1 == temp_2, temp_1 ~= temp_2] * [temp_2; -temp_2];
temp_1 = max(temp_Eu_im);
temp_2 = max(abs(temp_Eu_im));
e_Eu_impl_h = [temp_1 == temp_2, temp_1 ~= temp_2] * [temp_2; -temp_2];
temp_1 = max(temp_RuKu);
temp_2 = max(abs(temp_RuKu));
e_RuKu_h = [temp_1 == temp_2, temp_1 ~= temp_2] * [temp_2; -temp_2];
disp('Globale Diskretisierungsfehler mit optimierten ''h'':');
disp(' ');
disp(['           Vorgegebener Fehler          |       ',num2str(F_opt)]);
disp('        --------------------------------------------------------------');
disp(['        Euler-Explizit-Verfahren        |       ',num2str(e_Eu_expl_h)]);
disp(['           Anzahl der Schritte          |       ',num2str(AnzahlSchritte2)]);
disp('        --------------------------------------------------------------');
disp(['        Euler-Implizit-Verfahren        |       ',num2str(e_Eu_impl_h)]);
disp(['           Anzahl der Schritte          |       ',num2str(AnzahlSchritte3)]);
disp('        --------------------------------------------------------------');
disp(['          Runge-Kutta-Verfahren         |       ',num2str(e_RuKu_h)]);
disp(['           Anzahl der Schritte          |       ',num2str(AnzahlSchritte4)]);
