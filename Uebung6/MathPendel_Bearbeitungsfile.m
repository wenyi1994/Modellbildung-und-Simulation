...
%       - Figure 1
%           + Grafik Winkel mit Laufzeit
%           + Grafik Winkelgeschwindigkeit mit Laufzeit
%           + Grafik lokale Fehler mit Laufzeit
%           + Grafik glabale Fehler mit Laufzeit
%       - Figure 2
%           + Grafik Winkel mit Laufzeit
%  * Bei Optimierung von h wird nur Euler-explizit-Verfahren
%    durchgefuehrt, da die Rechnungszeit sehr lang ist.
%  * Die am Ende kommentierte Codes fuehren die lokale Fehler und globale
%    Fehler mit optiemierten h aus. ES KANN VIELE ZEIT VERBRAUCHEN.
%  ----------------------------------------------------------------------
...
ts=5;                               % s             %% Laufzeit
F_opt=0.03;                                        %% vorgegebener Fehler
...
disp('Berechnungszeit: numerische Verfahren');
...
while t2(t_count)<ts
    ...
    h_opt=h;
    delta=abs(x_Eu_expl2(1,t_count+1)-pi/6*cos(sqrt(g/l)*(t2(t_count)+h_opt)));
    while delta > F_opt
